...
- [x] Etapa 01 concluída (Node e Docker funcionando)
...
- [x] A pasta `projeto-docker-nodejs` existe
- [x] Dentro dela existem `database`, `public` e `src`
- [x] `src` contém `config`, `modules`, `routes` e `shared`
- [x] `src/modules` contém as 4 pastas dos módulos
- [x] O arquivo `package.json` existe e está salvo
- [x] O VS Code não mostra nenhum erro vermelho no `package.json`
- [x] Você **não** rodou `npm install`
