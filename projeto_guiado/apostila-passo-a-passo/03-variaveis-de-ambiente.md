...
- [x] Etapa 02 concluída (pastas criadas e `package.json` salvo)
...
- [x] O arquivo `.env.example` existe e tem 8 variáveis
- [x] O arquivo `.env` existe com o mesmo conteúdo
- [x] O arquivo `.gitignore` existe e contém `.env`
- [x] `DB_HOST` está como `db` (e **não** `localhost`)
- [x] Não há espaços ao redor dos sinais de `=`
