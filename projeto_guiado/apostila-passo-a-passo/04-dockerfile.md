...
- [x] Etapa 03 concluída (`.env` criado)
...
- [x] O arquivo se chama `Dockerfile` (D maiúsculo, sem extensão)
- [x] Tem 6 instruções: `FROM`, `WORKDIR`, `COPY`, `RUN`, `COPY`, `EXPOSE`, `CMD`
- [x] O `COPY package*.json` vem **antes** do `RUN npm install`
- [x] O `COPY . .` vem **depois** do `RUN npm install`
- [x] O `.dockerignore` existe e lista `node_modules`
